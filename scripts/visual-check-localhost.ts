...
async function visualCheck(url: string, siteName: string): Promise<VisualCheckResult> {
  ...
  const outputDir = path.join(process.cwd(), 'visual-checks', siteName, timestamp);
  ...
  try {
    ...
    mobilePage.on('console', msg => {
      if (msg.type() === 'error' || msg.type() === 'warning') {
        const errorMsg = `[${msg.type()}] ${msg.text()}`;
...
if (!url) {
  console.error('Usage: npx ts-node scripts/visual-check-localhost.ts <url> [siteName]');
  process.exit(1);
}
