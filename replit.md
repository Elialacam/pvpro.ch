...
- **Sitemap handler**: `app/api/sitemap/route.ts` — generates full XML with `xmlns:xhtml` namespace and `<xhtml:link hreflang>` tags (130 URLs, 4 locales each, includes photovoltaik-schweizer-klima group and 3 new blog articles). New articles with explicit entry4: `lohnt-sich-solaranlage-schweiz-2026`, `solaranlage-steuerabzug-schweiz-2026`, `solaranlage-waermepumpe-kombinieren-schweiz`.
